"""True batched CUDA C++ body kernels for V7-derived runtime V8."""
...
CUDA_SOURCE = ROOT / "experiments/cuda/wal_batched_body_gemv_v1.cu"
_EXTENSION = None
...
def direct_wal_linear_v8(
    ...
    if not 2 <= batch <= 16:
        raise ValueError("V8 fast path is restricted to batches 2..16")
    ...
    if columns != base_columns:
        raise ValueError("V8 base activation width mismatch")
    if not paths:
        output = extension().t3_sparse_batched(
            flat, t3, positions, sign_bits, alpha, beta,
            rows, columns, batch,
        )
        return output.reshape(*value.shape[:-1], rows)
    if len(paths) != 2:
        raise ValueError("V8 requires zero or two WALB2 paths")
    path0, path1 = paths
    ...
    if dimensions0 != dimensions1 or dimensions0[:2] != (rows, columns):
        raise ValueError("V8 base/overlay dimensions differ")
    rank = dimensions0[2]
    hidden0, hidden1 = extension().binary_pair_v_batched(
        flat, path0["v"], path1["v"], path0["latent"], path1["latent"],
        path0["column"], path1["column"], rank, columns, batch,
    )
    output = extension().t3_sparse_walb2_u_batched(
