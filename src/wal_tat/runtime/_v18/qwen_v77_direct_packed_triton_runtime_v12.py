"""Runtime V12: V10 with one-barrier batched decode reductions."""
...
import qwen_v77_direct_packed_cuda_operator_v12 as operator_v12
...
import qwen_v77_direct_packed_triton_runtime_v10 as runtime_v10
import qwen_v77_packed_reference_runtime as reference


class DirectPackedWALLinearV12(runtime_v10.DirectPackedWALLinearV10):
    @torch.no_grad()
    def forward(self, value: torch.Tensor) -> torch.Tensor:
        flattened = value.reshape(-1, value.shape[-1]).shape[0]
        if 2 <= flattened <= 16:
            base, paths = self._direct(value.device)
            return operator_v12.direct_wal_linear_v12(
                value, base, paths,
...
def load_direct_model(
    ...
    *, device: str = "cuda:0", verify_hashes: bool = True,
    ...
    DirectPackedWALLinearV12.hardware_cache_root = cache_path
    model, report = reference.load_packed_model(
        checkpoint_path, device=device, dtype=torch.bfloat16, row_chunk=256,
        verify_hashes=verify_hashes, wal_linear_class=DirectPackedWALLinearV12,
    ...
    report["decode_kernel"] = {
        "version": "cuda-v12-one-barrier-reduction",
        "batch_one": "accepted-fused-v7",
        "batch_2_to_16": "V10 weight reuse with one reduction barrier per stage",
