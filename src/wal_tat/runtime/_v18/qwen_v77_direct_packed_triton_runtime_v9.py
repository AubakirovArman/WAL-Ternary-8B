"""Runtime V9: V7/V8 decode plus temporary-unpack tensor-core prefill."""
...
import qwen_v77_direct_packed_prefill_v9 as prefill_v9
...
import qwen_v77_direct_packed_triton_runtime_v8 as runtime_v8
import qwen_v77_direct_packed_int4_head_v8 as head_v8
import qwen_v77_packed_reference_runtime as reference


class DirectPackedWALLinearV9(runtime_v8.DirectPackedWALLinearV8):
    @torch.no_grad()
    def forward(self, value: torch.Tensor) -> torch.Tensor:
        flattened = value.reshape(-1, value.shape[-1]).shape[0]
        if flattened > 16 and os.environ.get("WAL_PREFILL_V9", "1") != "0":
            base, paths = self._direct(value.device)
            return prefill_v9.direct_wal_linear_prefill_v9(
                value, base, paths,
...
def load_direct_model(
    ...
    *, device: str = "cuda:0", verify_hashes: bool = True,
    ...
    DirectPackedWALLinearV9.hardware_cache_root = cache_path
    model, report = reference.load_packed_model(
        checkpoint_path, device=device, dtype=torch.bfloat16, row_chunk=256,
        verify_hashes=verify_hashes, wal_linear_class=DirectPackedWALLinearV9,
    ...
    report["decode_kernel"] = {
        "version": "cuda-v9-prefill-plus-v8-decode",
        "batch_one": "accepted-fused-v7",
        "batch_2_to_16": "cuda-cpp-2d-grid-v8",
        "prefill_m_gt_16": "temporary current-operator unpack plus tensor-core GEMM",
